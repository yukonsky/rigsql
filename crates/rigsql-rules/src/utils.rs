    ...
    children.iter().any(|child| {
        if let Segment::Token(t) = child {
            t.segment_type == SegmentType::Keyword
                && t.token.text.eq_ignore_ascii_case("AS")
        } else {
            false
        }
    })
...
pub fn last_non_trivia(children: &[Segment]) -> Option<&Segment> {
    children.iter().rev().find(|c| !c.segment_type().is_trivia())
}

/// Keywords that should NOT be treated as alias names.
/// Sorted alphabetically for binary_search.
const NOT_ALIAS_KEYWORDS: &[&str] = &[
    "ALTER", "AND", "BEGIN", "BREAK", "CATCH", "CLOSE", "COMMIT",
    "CONTINUE", "CREATE", "CROSS", "CURSOR", "DEALLOCATE", "DECLARE",
    "DELETE", "DROP", "ELSE", "END", "EXCEPT", "EXEC", "EXECUTE",
    "FETCH", "FOR", "FROM", "FULL", "GO", "GOTO", "GROUP", "HAVING",
    "IF", "INNER", "INSERT", "INTERSECT", "INTO", "JOIN", "LEFT",
    "LIMIT", "MERGE", "NATURAL", "NEXT", "OFFSET", "ON", "OPEN", "OR",
    "ORDER", "OUTPUT", "OVER", "PRINT", "RAISERROR", "RETURN",
    "RETURNING", "RIGHT", "ROLLBACK", "SELECT", "SET", "TABLE", "THEN",
    "THROW", "TRUNCATE", "TRY", "UNION", "UPDATE", "VALUES", "WHEN",
    "WHERE", "WHILE", "WITH",
];
...
pub fn is_false_alias(children: &[Segment]) -> bool {
    ...
    if let Some(alias) = last_non_trivia(children) {
        if let Segment::Token(t) = alias {
...
pub fn find_keyword_in_children<'a>(children: &'a [Segment], name: &str) -> Option<(usize, &'a Segment)> {
    children.iter().enumerate().find(|(_, c)| {
        if let Segment::Token(t) = c {
            t.segment_type == SegmentType::Keyword
                && t.token.text.eq_ignore_ascii_case(name)
        } else {
            false
        }
    })
