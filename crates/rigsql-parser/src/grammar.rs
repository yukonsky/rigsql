...
impl Grammar {
    ...
    pub fn parse_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        ...
        } else if ctx.peek_keyword("DECLARE") {
    ...
    fn parse_declare_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
    ...
    fn parse_declare_variable(ctx: &mut ParseContext, children: &mut Vec<Segment>) {
        // @variable name or cursor_name
        if ctx.peek_kind() == Some(TokenKind::AtSign) {
            let at = ctx.advance().unwrap();
            children.push(Self::token_segment(at, SegmentType::Identifier));
            children.extend(Self::eat_trivia_segments(ctx));
            ...
            if ctx.peek_keyword("CURSOR") {
                children.push(Self::token_segment(name, SegmentType::Identifier));
                children.extend(trivia);
            } else {
                ctx.restore(save);
            }
        ...
        if ctx.peek_keyword("CURSOR") {
            ...
            children.push(Self::token_segment(cursor_kw, SegmentType::Keyword));
            children.extend(Self::eat_trivia_segments(ctx));
            ...
            while !ctx.at_eof() && !ctx.peek_keyword("FOR") {
                ...
                if ctx.peek_kind() == Some(TokenKind::Word) {
                    ...
                    children.push(Self::token_segment(opt, SegmentType::Keyword));
                    children.extend(Self::eat_trivia_segments(ctx));
                } else {
                    break;
                }
            ...
            if ctx.peek_keyword("FOR") {
                ...
                children.push(Self::token_segment(for_kw, SegmentType::Keyword));
                children.extend(Self::eat_trivia_segments(ctx));
                if let Some(sel) = Self::parse_select_statement(ctx) {
                    children.push(sel);
                }
        ...
        if ctx.peek_keyword("TABLE") {
            ...
            children.push(Self::token_segment(table_kw, SegmentType::Keyword));
            children.extend(Self::eat_trivia_segments(ctx));
            if ctx.peek_kind() == Some(TokenKind::LParen) {
                if let Some(defs) = Self::parse_paren_block(ctx) {
                    children.push(defs);
        ...
        if ctx.peek_kind() == Some(TokenKind::Eq) {
            ...
            children.push(Self::token_segment(eq, SegmentType::ComparisonOperator));
            children.extend(Self::eat_trivia_segments(ctx));
            if let Some(expr) = Self::parse_expression(ctx) {
                children.push(expr);
            }
    ...
    fn parse_set_variable_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let save = ctx.save();
        let mut children = Vec::new();
        let kw = ctx.eat_keyword("SET")?;
        children.push(Self::token_segment(kw, SegmentType::Keyword));
        ...
        if ctx.peek_kind() == Some(TokenKind::AtSign) {
            ...
            if let Some(kind) = ctx.peek_kind() {
                if matches!(
                    ...
                    let op = ctx.advance().unwrap();
                    children.push(Self::token_segment(op, SegmentType::Operator));
                    // Handle compound assignment: +=, -=, etc.
                    if ctx.peek_kind() == Some(TokenKind::Eq) {
                        let eq = ctx.advance().unwrap();
                        children.push(Self::token_segment(eq, SegmentType::Operator));
                    }
                    children.extend(Self::eat_trivia_segments(ctx));
                    if let Some(expr) = Self::parse_expression(ctx) {
                        children.push(expr);
    ...
    fn parse_if_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        ...
        if ctx.peek_keyword("ELSE") {
            ...
            children.push(Self::token_segment(else_kw, SegmentType::Keyword));
            children.extend(Self::eat_trivia_segments(ctx));
    ...
    fn parse_begin_block<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        ...
        children.push(Self::token_segment(begin_kw, SegmentType::Keyword));

        Self::parse_statement_list(ctx, &mut children, |c| c.peek_keyword("END"));
    ...
    fn parse_try_catch_block<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        ...
        children.push(Self::token_segment(try_kw, SegmentType::Keyword));

        Self::parse_statement_list(ctx, &mut children, |c| c.peek_keywords(&["END", "TRY"]));
    ...
    fn parse_while_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
    ...
    fn parse_exec_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        // EXEC or EXECUTE
    ...
    fn parse_exec_params(ctx: &mut ParseContext, children: &mut Vec<Segment>) {
    ...
    /// Parse RETURN [expr]
    fn parse_return_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
    ...
    /// Parse PRINT expr
    fn parse_print_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
    ...
    fn parse_throw_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        ...
        for _ in 0..2 {
            let save2 = ctx.save();
            let trivia2 = Self::eat_trivia_segments(ctx);
            if let Some(comma) = ctx.eat_kind(TokenKind::Comma) {
    ...
    fn parse_raiserror_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        ...
        if ctx.peek_keyword("WITH") {
            ...
            children.push(Self::token_segment(with_kw, SegmentType::Keyword));
            children.extend(Self::eat_trivia_segments(ctx));
    ...
    fn parse_go_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
    ...
    fn parse_simple_statement<'a>(ctx: &mut ParseContext<'a>) -> Option<Segment> {
        let mut children = Vec::new();
        let kw = ctx.advance()?;
        children.push(Self::token_segment(kw, SegmentType::Keyword));
        children.extend(Self::eat_trivia_segments(ctx));
    ...
    fn peek_statement_start(ctx: &ParseContext) -> bool {
        if let Some(t) = ctx.peek_non_trivia() {
            if t.kind == TokenKind::Word {
    ...
    fn consume_until_statement_end(ctx: &mut ParseContext, children: &mut Vec<Segment>) {
        while !ctx.at_eof() {
            if ctx.peek_kind() == Some(TokenKind::Semicolon) {
                break;
            }
